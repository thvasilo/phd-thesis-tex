...
in imbalances between the workers, leading to increased runtime\cite{staggler-ml, stale-sync-ps}.


The MapReduce programming paradigm offered some more flexibility in terms of the
programming model, in which the base primitives are a map operation that is applied
locally at worker on the input, and a reduce operation that aggregates the results
from worker that correspond to the same key. Developers can assign custom partitioners
to have more flexible communication, however the communication pattern is still rigid.
The use of disk-based aggregations of MapReduce are not a good fit for the iterative
nature of most ML algorithms and often lead to increased runtime\cite{distributed-ml-design}.
