...
to dramatically reduce the computational cost of exact algorithms, and develop
\emph{online learning} algorithms to deal with a constantly changing environment
under a tight computational budget. We design \emph{parallel and distributed} algorithms
to ensure that our methods can scale to massive datasets.

We first propose a scalable algorithm for graph vertex similarity calculation and concept
discovery. We demonstrate
its applicability to multiple domains, including text, music, and images, and demonstrate its scalability by training on
one of the largest text corpora available.
...
state of the art while being an order of magnitude faster.
...
magnitude speedup on average. Finally, we propose an algorithm that allows for gradient boosted trees training to be
