\chapter{Session Length Prediction using Gradient Boosted Trees}\todo{Whole chapter needs fixing, sloppy and too much copying.}

In this chapter we provide an overview of our work on session length
prediction for media streaming using gradient boosted trees. After giving
...
\ref{sec:session-length-main-findings}.

\section{Background}
\label{sec:session-length-background}

As streaming services have proliferated it's become important for streaming
companies to analyze their users' behavior to optimize their offering and
meet business. One important factor in user satisfaction is the length of
a user's sessions~\cite{dwell-time-satisfaction}, which we define as the
