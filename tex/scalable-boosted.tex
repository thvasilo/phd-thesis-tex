...
bit-strings. Each bit in the bit-string
...
(remember we move right when a node evaluates to false), but either of the
two right-most leaves could still be reached.

Let's take data point 2, which has the values $f1: 7, f2:1 , f3: 667$.
...
the exit node we simply need to perform an \texttt{AND} operation between these two and an all-ones
bit-string: $1111 \land 0011 \land 1101 = 0001$. The left-most bit set to one
...
\begin{figure*}
	...
	\ref{tab:example-data-reprod}. When a node condition evaluate to false, we move to
	the right child of the node.
	The bitstring  zeros indicate the leafs that become unreachable when a node a evaluates to false.}
...
So it can only evaluate the conditions of the tree that correspond to its slice
...
Then Worker 1 would be able to only evaluate the condition of the root as false and produce
...
many of the histogram values can be zero as can be seen in Figure \ref{label}, where we show
...
subset of $F$ (features). In \blockgbt, we use a sparse representation of this tensor at each worker, which allows
...
implementation of MOA (Shown in Table 2 of the paper).
...
the sparse communication we employ over the dense row-distributed approach, we use 4 datasets with different sparsity levels. First
...
Finally we demonstrate the communication savings possible in terms of the feature sketches. As we mentioned in Section \ref{sec:bg-dt-gbts}, in order to determine the possible
