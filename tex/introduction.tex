...
Decision tree have been shown to perform well for a wide variety of tasks~\cite{hundreds-classifiers}
...
manner, and hyperparameters that are straightforward to tune.
%Comparing decision trees with deep learning models mentioned before, the space
%of architecture search in a deep network versus parameters like
%the minimum splitting weight in a tree is much larger, meaning
%that designing a neural network for a specific task requires
%much more domain knowledge and experience than training a
...
however, their time and space costs have been derived where possible.
...
\begin{itemize}
	...
	for similarity discovery~\cite{concepts-icdm}. \\
	...
	\item \textbf{\sessionlength}: Predicting Session Length in Media Streaming~\cite{session-length}.
	...
	\item \textbf{\boostvht}: BoostVHT: Boosting Distributed Streaming Decision Trees~\cite{boost-vht}.
	...
	\item \textbf{\uncertaintrees}: Quantifying Uncertainty in Online Regression Forests~\cite{uncertain-trees}.
	...
	\item \textbf{\blockgbt}: Block-distributed Gradient Boosted Trees~\cite{block-gbt}.
